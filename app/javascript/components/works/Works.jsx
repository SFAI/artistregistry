...
class Works extends React.Component {
  ...
  componentDidMount = () => {
    ...
    Promise.all([
      ...
      this.setState({
        ...
        isLoading: false,
        pageCount: Math.ceil(
          works_response.work_count / works_response.per_page
        ),
      });
    });
  };
  ...
  updateFeatured = (work_id, unhide = false) => {
    ...
    if (unhide == true) {
      ...
      let newFeaturedWork = this.state.works
        .filter(work => work.artist.id == artist.id)
  ...
  render() {
    ...
    return (
      <div className="pt4">
        {isLoading ? (
          <LoadingOverlay itemType="artwork" fullPage={true} />
        ) : null}
        <div className="fl w-20 pa3 mt5" role="search">
          ...
          />
          <button
            onClick={() => this.getFilteredWorks(1)}
            className="button-primary bg-berry w-100"
          >
            Apply
          </button>
        </div>
